...
bool check_parentheses(size_t p, size_t q)
{
  ...
  {
    if (tokens[idx].type != '(' && tokens[idx].type != ')')
      continue;
    if (tokens[idx].type == '(')
      ++sum;
    else
...
size_t find_main_op(size_t p, size_t q)
{
  ...
  for (size_t idx = p; idx <= q; ++idx)
  {
    if (tokens[idx].type == NUM || tokens[idx].type == REG)
      continue;
    if (tokens[idx].type == '(')
    {
      while (idx <= q && tokens[idx].type != ')')
        ++idx;
    }
    // 上面逻辑使得遇到)跳过，所以不会出现)
    else
      ...
      else
      {
        ...
        for (int i = 0; i < 6; ++i)
        {
          if (tokens[re_idx].type == op_priority[i].op)
            mainprio = op_priority[i].prio;
          if (tokens[idx].type == op_priority[i].op)
            nowprio = op_priority[i].prio;
        }
        if (nowprio <= mainprio)
          re_idx = idx;
      }
